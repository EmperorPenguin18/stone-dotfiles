...
sudo pacman -Sy git base-devel meson --noconfirm --needed
git clone https://github.com/EmperorPenguin18/stone-dotfiles $DIR
cd $DIR
...
build_mesa ()
{
  sudo pacman -S python-mako llvm wayland-protocols xorg-xrandr --asdeps --noconfirm --needed
  git clone -b dev/pi_drm_format https://gitlab.freedesktop.org/EmperorPenguin18/mesa.git
  cd mesa
  ...
  sudo ninja $NINJAFLAGS -C build install && \
  sudo rm -f /usr/bin/mesa-overlay-control.py && \
  sudo ln -sf /usr/lib/libGLX_mesa.so.0 /usr/lib/libGLX_indirect.so.0 || \
  return 1
  cd ../ && return 0
}
...
build_ffmpeg ()
{
  sudo pacman -S libepoxy --asdeps --noconfirm --needed
  git clone -b dev/5.1.2/rpi_import_1 https://github.com/EmperorPenguin18/rpi-ffmpeg
  cd rpi-ffmpeg
...
build_mpv ()
{
  sudo pacman -S sdl2 wireplumber pipewire-jack hicolor-icon-theme luajit wayland-protocols yt-dlp libass --asdeps --noconfirm --needed
  git clone -b pi_h265 https://github.com/EmperorPenguin18/mpv
...
#Auto-login as user
sudo cp "$DIR/override.conf" "/etc/systemd/system/getty@tty1.service.d/"
...
#Youtube
git clone https://github.com/CogentRedTester/mpv-scroll-list $DIR/mpv-scroll-list
dotfile "$DIR/mpv-scroll-list/scroll-list.lua" "/home/$USER/.config/mpv/script-modules/"
git clone https://github.com/CogentRedTester/mpv-user-input $DIR/mpv-user-input
...
git clone https://github.com/CogentRedTester/mpv-scripts $DIR/mpv-scripts
