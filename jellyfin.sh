...
then
	...
	SHOWS=$(curl -sX "GET" "http://$URL/Items?api_key=$KEY&userID=$USERID&parentId=$LIBRARYID" | jq ".Items[]")
	FIXEDTITLE=$(echo "$SHOWS" | sed 's/:/@/g' | grep "$(echo $TITLE | sed 's/-/@/g')" | cut -d '"' -f 4 | sed 's/@/:/g')
	TITLEID=$(echo $SHOWS | jq "select(.Name==\"$FIXEDTITLE\") | .Id" | cut -d '"' -f 2)
